...
public interface BarcodeTypeV1Controller {
  ...
  @ApiResponse(responseCode = "500", description = "A internal erros has ocurred, detaisl in the error message", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ProblemDetail.class)))
  @GetMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
  @ResponseStatus(HttpStatus.OK)
  @Secured({Roles.ROLE_USER, Roles.ROLE_ADMIN})
  BarcodeTypeGetByIdResponse findById(
      @Parameter(in = ParameterIn.PATH, name = "ID", description = "UUID (Universally Unique Identifier) of the Barcode Type", example = "c2e559b6-e4e3-489a-bb30-a2666fc4e1d4") @PathVariable("id") @Valid @NotNull @org.hibernate.validator.constraints.UUID(version = 4, letterCase = org.hibernate.validator.constraints.UUID.LetterCase.INSENSITIVE) UUID externalId);
  ...
  @ApiResponse(responseCode = "500", description = "A internal erros has ocurred, details in the error message", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ProblemDetail.class)))
  ...
  @ApiResponse(responseCode = "404", description = "No BarcodeType found with the supplied uuid", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ProblemDetail.class)))
